...
export function CronPage(): React.ReactElement {
	...
	const toggleHistory = async (jobId: string) => {
		...
		try {
			setHistoryRuns(
				(await apiGet(`/api/cron/runs?jobId=${jobId}`)) as typeof historyRuns,
			);
		} catch (e) {
			setErr(String(e));
		}
	...
	return (
		<div>
			...
			>
				Supercronic — crontab at <code>/data/crontab</code>. Webhook jobs are
				triggered via{" "}
				<code>POST :{webhookPort}/hook/&lt;id&gt;/&lt;token&gt;</code>.
			</p>
			<div
				className="card"
			...
			>
				<label style={labelStyle}>
					<span style={labelHeadStyle}>Name</span>
					<input
						aria-label="Job name"
				...
				{kind === "prompt" && (
					<label style={labelStyle}>
						<span style={labelHeadStyle}>Prompt</span>
						<textarea
							aria-label="Prompt"
							placeholder="prompt for omp --mode json -p"
							value={prompt}
				...
				{kind === "script" && (
					<>
						<label style={labelStyle}>
							<span style={labelHeadStyle}>Script source</span>
							<select
								aria-label="Script source"
								value={scriptSource}
								onChange={(e) =>
									setScriptSource(e.target.value as "inline" | "file")
								}
							>
						...
						{scriptSource === "inline" ? (
							<label style={labelStyle}>
								<span style={labelHeadStyle}>Script (bash)</span>
								<textarea
									aria-label="Script"
							...
							<label style={labelStyle}>
								<span style={labelHeadStyle}>Script file (absolute path)</span>
								<input
									aria-label="Script file"
									placeholder="/opt/scripts/deploy.sh"
									value={script}
									onChange={(e) => setScript(e.target.value)}
								/>
							</label>
						...
						<label style={labelStyle}>
							<span style={labelHeadStyle}>Args (comma-separated)</span>
							<input
				...
				{trigger === "schedule" && (
					<label style={labelStyle}>
						<span style={labelHeadStyle}>Cron</span>
						<input
				...
				<label style={labelStyle}>
					<span style={labelHeadStyle}>Project</span>
					<select
						aria-label="Project"
						value={projectId}
				...
				<label style={labelStyle}>
					<span style={labelHeadStyle}>Model (optional)</span>
					<ModelSelect value={model} onChange={setModel} allowNone />
				</label>
				{err && (
							...
							{editing === j.id ? (
								...
								<>
									<span style={{ flex: 1, minWidth: 180 }}>
										...
										<span
											className={`badge ${j.enabled ? "badge-success" : ""}`}
										>
											{j.enabled ? "enabled" : "disabled"}
										</span>{" "}
										{j.model ? <span className="badge">{j.model}</span> : null}
									</span>
									{j.trigger === "webhook" && j.webhookPath && (
										<>
											<code
												...
												title={`http://<host>:${webhookPort}${j.webhookPath}`}
											>
												{j.webhookPath}
											</code>
											<button
									...
									>
										{j.kind === "script" ? (
											...
											>
												{j.script_source === "file"
													? j.script
													: (j.script ?? "").slice(0, 160)}
											</code>
										) : (
											...
											{historyRuns.length === 0 ? (
												...
												historyRuns.slice(0, 20).map((r) => (
													...
													>
														...
														>
															<span
																className={`badge ${r.status === "success" ? "badge-success" : r.status === "error" ? "badge-error" : r.status === "running" ? "badge-running" : "badge-warn"}`}
															>
																{r.status}
															</span>
															<span
