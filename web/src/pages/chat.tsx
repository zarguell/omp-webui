...
function summarizeToolCall(
	...
	startData?: { intent?: string; args?: Record<string, unknown> },
): string {
	const a = args ?? {};
	...
	switch (name) {
		case "read":
			return `→ read ${shortPath(String(a.path ?? ""))}${intent ? ` — ${intent}` : ""}`;
		case "edit": {
		...
		case "write":
			return `→ write ${shortPath(String(a.path ?? ""))}`;
		case "grep":
			return `→ grep ${String(a.pattern ?? "").slice(0, 60)}${a.path ? ` in ${shortPath(String(a.path))}` : ""}`;
		case "glob":
			return `→ glob ${String(a.path ?? a.i ?? "").slice(0, 60)}`;
		case "lsp":
			return `→ lsp ${a.action ?? ""}${a.file ? ` ${shortPath(String(a.file))}` : ""}${a.symbol ? ` #${a.symbol}` : ""}`;
		case "ast_edit":
			return `→ ast_edit ${shortPath(String((a.paths as string[])?.[0] ?? ""))}`;
		case "task":
			return `→ task${a.i ? ` — ${String(a.i).slice(0, 80)}` : ""}`;
		case "todo":
			return `→ todo ${a.op}${a.task ? ` — ${String(a.task).slice(0, 60)}` : ""}`;
		case "debug":
			return `→ debug ${a.action}${a.program ? ` ${shortPath(String(a.program))}` : ""}`;
		case "browser":
			return `→ browser ${a.action}${a.url ? ` ${String(a.url).slice(0, 60)}` : ""}`;
		default:
...
function formatToolDetail(name: string, args: Record<string, unknown>): string {
	...
	switch (name) {
		...
		case "grep":
			return `pattern: ${a.pattern ?? ""}${a.path ? `\npath: ${a.path}` : ""}`;
		case "glob":
			return `path: ${a.path ?? a.i ?? ""}`;
		case "lsp":
			return `action: ${a.action ?? ""}${a.file ? `\nfile: ${a.file}` : ""}${a.symbol ? `\nsymbol: ${a.symbol}` : ""}${a.line ? `\nline: ${a.line}` : ""}`;
		case "ast_edit":
			return JSON.stringify(a, null, 2).slice(0, 1000);
		case "task":
			return JSON.stringify(a, null, 2).slice(0, 1000);
		default:
			return JSON.stringify(a, null, 2).slice(0, 1000);
	...
	for (const line of lines) {
		try {
			...
			if (entry.customType === "tool_execution_start" && entry.data) {
				...
				if (d.toolCallId)
					toolStarts.set(d.toolCallId, { intent: d.intent, args: d.args });
			}
		} catch {}
	}
		...
		return (
			...
			>
				...
				>
					...
					<span
						style={{
							color: isCmd ? "var(--primary)" : "var(--text)",
							fontWeight: 500,
							flex: 1,
						}}
					>
...
function MessageBubble({ msg }: { msg: ChatMessage }): React.ReactElement {
	...
	> = {
		user: {
			...
			border: "var(--border)",
			badge: "var(--primary)",
			badgeBg: "oklch(0.95 0.05 285)",
			badgeBorder: "oklch(0.86 0.08 285)",
		},
		assistant: {
			bg: "var(--panel)",
			border: "var(--border)",
			badge: "var(--success)",
			badgeBg: "oklch(0.96 0.04 145)",
			badgeBorder: "oklch(0.88 0.06 145)",
		},
		system: {
			bg: "var(--panel)",
...
export function ChatPage({
	...
	useEffect(() => {
		void apiGet("/api/models")
			.then((data: unknown) => {
				const d = data as {
					models?: { selector: string; name: string; provider: string }[];
				};
				if (d.models)
					setAvailableModels(
						d.models.map((m) => ({
			...
			.catch(() => {});
		void apiGet(`/api/sessions/${sessionId}/model`)
			.then((data: unknown) => {
		...
		>
			...
			>
				...
				{model && (
					...
					>
						<option value={model}>
							{availableModels.find((m) => m.selector === model)?.provider}/
							{availableModels.find((m) => m.selector === model)?.name ??
								model.split("/").pop()}
						</option>
						{availableModels
							.filter((m) => m.selector !== model)
							.map((m) => (
